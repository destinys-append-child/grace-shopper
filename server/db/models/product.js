const Sequelize = require('sequelize')
const db = require('../db')

const Product = db.define('product', {
  name: {
    type: Sequelize.STRING,
    unique: true,
    allowNull: false,
    validate: {
      notEmpty: true
    }
  },
  imageUrl: { // missing validations
    type: Sequelize.STRING, // be careful with this. I don't think this default value will work
    defaultValue:
      'data:image/png;base64,'
  },
  description: {
    type: Sequelize.TEXT,
    allowNull: false,
    validate: {
      notEmpty: true
    }
  },
  price: {
    type: Sequelize.FLOAT,
    allowNull: false, // be vewwwwwwyyyyyyyyy careful with calculating prices
    validate: { // toFixed() is not good enough here
      min: 0.0
    }
  },
  avgRating: {
    type: Sequelize.FLOAT, // possibly a default value?
    validate: { // null values make me uncomfortable
      min: 0.0,
      max: 5.0
    }
  },
  quantity: {
    type: Sequelize.INTEGER,
    defaultValue: 10, //this is temporary!
    validate: {
      min: 0
    }
  },
  category: {
    type: Sequelize.STRING,
    allowNull: false,
    validate: {
      notEmpty: true
    }
  }
})

module.exports = Product
